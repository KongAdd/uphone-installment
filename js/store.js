...
const SHEET_FIELD_MAP = {
  ...
  currentAddressMapUrl: 'ลิงก์แผนที่ที่อยู่ปัจจุบัน',
  occupation: 'อาชีพ',
  salary: 'เงินเดือน',
  workplace: 'ที่ทำงาน',
...
function addContract(contract, opts) {
  ...
  const record = {
    ...
    currentAddress: (contract.currentAddress || '').trim(),
    currentAddressMapUrl: (contract.currentAddressMapUrl || '').trim(),
...
function exportContractsCSV() {
  ...
  const headers = [
    ...
    'ที่อยู่ปัจจุบัน', 'ลิงก์แผนที่ที่อยู่ปัจจุบัน',
  ...
  const rows = getContracts().map((c) => {
    ...
    return [
      ...
      c.currentAddress || '', c.currentAddressMapUrl || '',
...
function importContractsJSON(fileText) {
  ...
  const cleaned = contracts.map((c) => {
    ...
    return {
      ...
      currentAddressMapUrl: c.currentAddressMapUrl || '',
